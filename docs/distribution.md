...
| [dsh-market.com](https://dsh-market.com/) workshop | `dsh-session-insights` (category `tools`) | Merged to `dev` | Community-index registration [PR #1055](https://github.com/zhu1090093659/dsh-web-ui/pull/1055) merged into `dev` (merge commit `05c57c8a`, 2026-08-23); entry read back on `dev` (37 entries). The entry reaches dsh-market.com when `dev` is merged to `main` and `deploy-market.yml` runs on the `market/**` change — public site readback remains pending |
