...
export function parseSnapshot(snapshot, opts, coverage) {
  ...
  for (const r of snapshot.events) {
    ...
    if (type === 'turn/start' && d.turn != null) {
      ...
      if (String(d.name).toLowerCase() === 'session-insights')
        ...
        : s.patches
          ...
          : s.web_searches || s.mcp_calls
            ...
            : s.tool_calls >= 10
              ...
              : 'conversation'
  s.meta_analysis ||=
    s.skill_mentions.has('dsh-session-insights') ||
    (/insights/i.test(s.first) &&
      /使用洞察|semantic_insights|analyze_dsh_sessions|\$dsh-session-insights/i.test(
        s.first,
      ))
  return s
}
